...
server behind `#[@External.<targert>(...)]` host calls. Reached via `from "server"`; **not**
...
└── src/               ← see src/AGENTS.md
    ...
    ├── server.bp      ← `#[@External.<targert>(...)]` decls binding `server.mjs` (`serverServe`/`serverStop`)
...
- The `#[@External.<targert>(...)]` path is the **sibling** `./server.mjs`; the CLI ships the
...
- Host state / IO behind `#[@External.<targert>(...)]` in `server.mjs`; the core never sees it.
