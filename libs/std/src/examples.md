...
> template now live in their own package (a workspace sibling), reached with
> `import {…} from "erika"`. See [`../../../../erika/`](../../../../erika/)
> for its docs and examples.

## Real-world examples

### env-driven CLI smoke (commonJS + erlang)

A walkthrough mixing `process` + `env` + `os` + `time` + `json`. Each
fn is one host hop; the composition is pure botopink.

```botopink
import {env, os, process, time, json, asserts} from "std";

fn main() {
    val pid = process.pid();
    val host = os.hostname();
    val cpus = os.cpuCount();
    val home = env.read("HOME").unwrapOr("(unset)");
    val started = time.nowMillis();
    val report = """{"pid":""" + pid.toString() + ""","host":""" + host + """","cpus":""" + cpus.toString() + """","home":""" + home + ""","started":""" + started.toString() + """}""";
    val normalized = json.parse(report).unwrapOr("{}");
    @print(normalized);
}
```

References:
- `env.read(name) -> ?string` → `libs/std/src/env.bp:24`
- `os.hostname() / cpuCount() / userInfo() / eol()` → `libs/std/src/os.bp`
- `process.pid() / cwd() / platform() / arch() / exit()` → `libs/std/src/process.bp`
- `time.nowMillis() / monotonicMillis() / measureMillis<T> / formatIso8601` → `libs/std/src/time.bp`
- `json.parse(s) -> @Result<string, string>` → `libs/std/src/json.bp:31`
- `asserts.matches(pattern, actual) / throws(body, message)` → `libs/std/src/asserts.bp`

> **Still pending (std-tail tail).** `fs.readText(path) -> @Result<string,
> string>` (F6.fs, gated on §A3 + sidecar shipping P10) and
> `http.get(url) -> *@Result<Response, string>` (F8.http, gated on sidecar
> shipping) round out the canonical "CLI tool reading config + a remote
> endpoint" walkthrough. The §A3 wrapper itself landed in std-tail P1;
> the consumer-side modules ride the next pass.

## Per-target coverage

Each std module's `#[@External.<target>( …)]` annotations drive whether
its declares lower on a given target. The matrix below is the V1
snapshot — every module ships on `commonJS` + `erlang`, with `wat`/`beam`
gated module-by-module. Cells: ✓ = lowers and tests pass; · = no
`#[@External.<targert>(...)]` for that target (callable, but lowers fall back to pure
botopink); ✗ = declared but red on inline tests.

| Module | commonJS | erlang | beam | wat |
|---|---|---|---|---|
| `order` | · | · | · | · |
| `dict` | · | · | · | · |
| `sets` | · | · | · | · |
...
The lookup eventually feeds the `STD-001` diagnostic (registered in
`comptime/diagnostics.zig` as `std-unsupported-on-target`) — once
`Env.target` threading lands, a `--target wat` build that imports
`{fs}` reds at the import span instead of failing at the first host
call. The diagnostic spelling is stable; the fire site is std-tail P9
runtime check.
