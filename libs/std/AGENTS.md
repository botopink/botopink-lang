...
| `http` | `type Response`, `fetch(url) -> @Task<@Result<Response, string>>` (a transport failure is `Error(message)` — the Node template catches the rejection, the Erlang one maps `{error, R}`, decision 126; an HTTP error status is an `Ok` response), `fetchStatus` |
