...
`collections` constructor is called on its type (`Dict.empty()`,
`Set.fromList(xs)`, `Queue.empty()` — decision 111). The table names each module
by its import path.
...
`import {<dir>} from "std"` is `unknown "std" module` (decision 110's folder
namespace — `import {io} from "std"; io.fs.readText(p)` — is not implemented).
