...
One-line installers from v0.beta.18 onwards:
...
Both scripts populate `$BPMP_HOME` (default `~/.bpmp` / `%USERPROFILE%\.bpmp`)
with `botopink`, `botopink-lsp`, `botopink-lib-test`, and `bpmp`. Add
`$BPMP_HOME/bin` to your `PATH`; the post-install hint prints the right
incantation per shell.

For **reproducible** installs (CI, container images), pin a version with
`BOTOPINK_VERSION`:
...
`$BOTOPINK_INSTALL_DIR` overrides `$BPMP_HOME`, `--quiet` suppresses
non-error output, and `--no-modify-path` (the default) keeps the shell
rc files untouched. Full flag/env reference lives in
[`scripts/AGENTS.md`](scripts/AGENTS.md).

After install, you manage everything through `bpmp` —
[`modules/bpmp/docs.md`](modules/bpmp/docs.md) walks the
`init → install → run` tutorial; the manifest schema is in
...
`#[@…]` form used for `#[@External.<targert>(...)]`. The annotation marks the implementation;
...
> The bare `*fn` prefix used to spell an effect (its kind was inferred from the
> return wrapper). It was deprecated in v0.beta.12 and **removed** in v0.beta.19;
> the parser now rejects it with a `deprecated-star-fn` diagnostic that maps the
> return wrapper to the canonical `#[@<effect>]` annotation.
...
### Sections — grouping variants inside an enum

A section is a named bag of variants nested inside an enum body. The bag
is opened with `Identifier { … }` (no keyword) — the parser picks `{`
as the discriminator vs `(` (payload) and `,`/`}` (bare). Sections may
nest arbitrarily deep, may sit next to bare/payload variants at any
level, and accept pure-digit variant names (`100`, `500`) inside the
section body (top-level enum bodies still reject digit names).
...
Path access reads each level with a single `.`:

```botopink
val tokens = [
    .Text.Bold,             // 2 segments
    .Text.Size.X3xl,        // 3 segments
    .Color.Red.500,         // numeric leaf
    .Color.Hex("#abc123"),  // 2 segments — payload variant
    .Hover([.Text.Underline]),
]
```

The comptime desugars each section into a synthesised inner enum
(`__Token__Color`, `__Token__Color__Red`, …) and surfaces one wrapper
variant per section on the parent (`Color(_inner: __Token__Color)`).
Codegen emits the inner enums as ordinary top-level enums and the
parent's wrapper ctor routes through them — byte-identical to the
hand-written enum-of-enum form, zero runtime cost.

Sections never carry methods themselves (methods live on the parent
enum and dispatch on its top-level wrappers via `match`). Numeric leaves
cannot take payload — for a string taxonomy use a `Hex(string)` payload
variant alongside the numeric section.
