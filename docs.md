...
|---|---|---|
| `break <value>` making the loop an expression — superseded by decision 105: **nothing evaluates to `[v]` any more**, `break v` and `yield v` need a generator scope (`#[@generator]` on the fn or on the `loop`), and a plain `loop` / `while` / `for` is a `void` statement | the loop's value is a **list** holding it: `val v = loop (0..10) { i -> if (i == 3) { break i; }; };` prints `[3]` on commonJS, on erlang and on wasm alike | 1.0.10-beta C-30 (`22-loops`, decision 105) — the three keywords, and the lowering of every backend |
...
| A pattern range written `..` and exclusive, as in a loop — `...` leaves the grammar | inverted: `1..9` in an arm reds `error[pattern-range-exclusive]` ("write `...` — an inclusive range, both ends matched"), and `1...9` is accepted. As a value the backends still disagree: `case 9 { 1...9 { 1 } _ { 0 } }` prints `1` on commonJS and on wasm (C-06's wasm half landed; it printed `256` before) and `0` on erlang | 1.0.10-beta C-06 (`02-erlang` step 3, `03-beam` step 3; decision 53 at run time). Decision 105 keeps both spellings and gives `a...b` a value in a `for` as well |
...
commonJS, and `loop (b.iter())` walks it there as it already did on erlang. Each
