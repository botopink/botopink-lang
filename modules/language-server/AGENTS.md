...
- `from "<lib>"` → the lib's own `botopink.json` (`src` + `files`), read from the
  first entry of the resolved **root list** (`resolveRoots`: `BOTOPINK_LIB_ROOTS`
  env entries → bundled `repository/botopink-lang/libs` → sibling `repository/`
  → legacy flat `libs/`, de-duped first-occurrence-wins) that carries `<lib>`. On
  the flat tree with the env unset the list is `[<ancestor>/libs]`, byte-identical
  to the former single-root walk. `.d.bp` declaration files are kept for
  go-to-def but excluded from the compile (the CLI drops them too).
...
- **`recv._N` (tuple-element chain — §E E1)** — `resolveHead` keeps a tuple
  binding's element types in `ReceiverType.tuple`; `stepField`'s `_<digits>`
...
- **`Iface.method(...)` (interface assoc-fn — §E E2)** — the head identifier
  ...
  `findInterfaceMethodAcross`, which scans the active file then the project
  graph (`others`) for `interface <head> { … }` and returns the inner
  `default fn <member>` / `declare fn <member>` token location. The
  interface must be `pub` to be reachable cross-module (mirrors
  `findMemberInTokens`'s `require_pub` gate).
...
|                       |                                          | CLI driver — server and CLI MUST see the same root list or `goto def`   |
|                       |                                          | misroutes when bpmp is in play.                                         |
| `XDG_CACHE_HOME` / `HOME` | `src/server.zig:computeTemplateRoot` | Template-eval scratch root for `@ExprCustom` expansion.                 |
...
- Non-existent entries silently dropped (a typo must not break the LSP).
- Empty entries and a relative path resolved against the process cwd.
- Unset / empty value → byte-identical to the legacy walk-up.
- Threaded via `Server.init(environ_map)` → `ProjectGraph.init(env_map)`;
  test code passes `null`.
