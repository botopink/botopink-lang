...
const ReceiverType = union(enum) {
    ...
    user: []const u8,
    /// `#(A, B, …)` value — element types kept so a chained `_N` step
    /// (§E E1) can resolve `t._0.field` against `A`'s declarations.
    tuple: []*T.Type,
    unknown,
};

/// Lifts a `*T.Type` (a binding's inferred type or a tuple element type)
/// into the LSP's `ReceiverType` lattice. `.named` becomes builtin/user/
/// tuple; everything else stays `.unknown` (the LSP chain walker bails).
fn receiverFromType(ty: *T.Type) ReceiverType {
    const t = ty.deref();
    if (t.* != .named) return .unknown;
    const n = t.named;
    if (std.mem.eql(u8, n.name, "tuple")) return .{ .tuple = n.args };
    if (builtinForName(n.name)) |bi| return .{ .builtin = bi };
    return .{ .user = n.name };
}
...
pub fn definitionMember(
    ...
    if (span.start > 0 and source[span.start - 1] == '.') {
        ...
        switch (resolveChainType(arena, source, tokens, pos, bindings, segs)) {
            ...
            // `_N` on a tuple receiver has no source-declared name (the position
            // is implicit). Resolution still flows through `resolveChainType`
            // so a downstream `t._0.field` can land on `field` — but a request
            ...
            .unknown => {
                ...
                // jump to its `default fn <name>` (or `declare fn <name>`).
                if (segs.len == 1) {
                    if (try findInterfaceMethodAcross(gpa, uri, tokens, segs[0], name, others)) |loc|
                        return .{ .location = loc };
                }
                return null;
            },
...
fn resolveHead(
    ...
    // A value binding → its inferred type. `#(A, B, …)` keeps its element
    // types so a chained `._N` step (§E E1) can narrow further.
    for (bindings) |b| {
        if (!std.mem.eql(u8, b.name, head)) continue;
        return receiverFromType(b.type_);
    }
    return .unknown;
}

/// Narrows `cur` by one `.field` step:
///   - `.tuple` + `_N` → element type N (§E E1).
///   - `.user` + named field → field's declared type (lookup in record/struct).
///   - else → unknown (the LSP chain walker bails).
fn stepField(
    bindings: []const comptime_pipeline.TypedBinding,
    cur: ReceiverType,
    field: []const u8,
    ...
    if (cur == .tuple) {
        // §E E1 — `recv._N` picks the Nth element type of a `#(A, B, …)`
        // value. The element's type drives any further chain step
        // (`t._0.field` resolves `field` on element 0's type).
        const idx = tupleMemberIndex(field) orelse return .unknown;
        if (idx >= cur.tuple.len) return .unknown;
        return receiverFromType(cur.tuple[idx]);
    }
...
/// §E E2 — Locates an interface method named `member` declared inside
/// `interface <iface_name> { … }`. Scans the active file first, then the
/// project graph deps (where the interface must be `pub` to be reachable).
/// Returns the `default fn <member>` or `declare fn <member>` token location.
fn findInterfaceMethodAcross(
    gpa: std.mem.Allocator,
    active_uri: []const u8,
    active_tokens: []const Token,
    iface_name: []const u8,
    ...
    if (try findInterfaceMethodInTokens(gpa, active_uri, active_tokens, iface_name, member, false)) |loc| return loc;
    for (others) |m| {
        if (std.mem.eql(u8, m.uri, active_uri)) continue;
        var arena = std.heap.ArenaAllocator.init(gpa);
        defer arena.deinit();
        var lexer = Lexer.init(m.source);
        const mtoks = lexer.scanAll(arena.allocator()) catch continue;
        if (try findInterfaceMethodInTokens(gpa, m.uri, mtoks, iface_name, member, true)) |loc| return loc;
    }
    return null;
}

/// Scans `tokens` for `interface <iface_name> { … }` and returns the location
/// of the `default fn <member>` / `declare fn <member>` declaration in its
/// body. When `require_pub`, the interface must be preceded by `pub`
/// (cross-module reachability).
fn findInterfaceMethodInTokens(
    gpa: std.mem.Allocator,
    uri: []const u8,
    tokens: []const Token,
    iface_name: []const u8,
    ...
    while (i < tokens.len) : (i += 1) {
        ...
        while (j < tokens.len and tokens[j].kind == .endOfFile) : (j += 1) {}
        if (j >= tokens.len or tokens[j].kind != .identifier) continue;
        if (!std.mem.eql(u8, tokens[j].lexeme, iface_name)) continue;
        ...
        while (q < body.close) : (q += 1) {
            ...
            // `default fn <member>` / `declare fn <member>` / bare `fn <member>`
            // — only the name token matters for the jump target.
            if (k == .@"fn") {
                var n = q + 1;
        ...
        // The interface was found but the member is not in its body: stop.
        return null;
    }
    return null;
}
...
/// the embedded interface `source`. Follows the `extends` chain so inherited
/// methods (e.g. `toString` on a numeric) resolve — `collectInterfaceMembersCached`
/// already walks the chain and caches the full member list per interface, so
/// this is now an O(N) array scan over a process-lifetime slice (no re-lex of
/// `iface.source`).
fn findInterfaceMemberRange(arena: std.mem.Allocator, iface: BuiltinInterface, member: []const u8) ?proto.Range {
    _ = arena;
    const members_opt = collectInterfaceMembersCached(iface) catch return null;
    const members = members_opt orelse return null;
    for (members) |m| {
        if (!std.mem.eql(u8, m.name, member)) continue;
        return .{
            .start = lsp_types.locToPosition(m.name_line, m.name_col),
            .end = lsp_types.locToPosition(m.name_line, m.name_col + m.name.len),
        };
    }
    return null;
}
...
fn builtinMethodSignature(
    ...
    const members = (try collectInterfaceMembersCached(iface)) orelse return null;
...
/// `name_line` / `name_col` are 1-based coords of the name token inside
/// `iface.source` — used by `findInterfaceMemberRange` to jump to the embedded
...
// same staleness profile as the `stdlib_template` lazy init.
//
// Hot path (cache hit): hash-lookup by interface name → O(1).
// Cold path (cache miss): one-time lex + walk into a process-lifetime
// arena, the result is reused forever. The returned slice's `.sig` fields
// point into the embedded source (static lifetime), so the slice is valid
// across any caller's arena lifetime.
//
// Without this cache, every completion / hover / signature_help on a
// builtin receiver (`42.`, `true.`, `xs.`, `"s".`) paid ~2–7ms re-lexing
// hundreds of lines of primitives source.

var iface_member_cache: std.StringHashMapUnmanaged([]const InterfaceMember) = .empty;
var iface_member_arena: std.heap.ArenaAllocator = undefined;
var iface_member_init: std.atomic.Value(u8) = .init(0);
var iface_member_mu: std.atomic.Value(u8) = .init(0);

fn ifaceMemberInit() void {
    while (true) {
        const s = iface_member_init.load(.acquire);
        if (s == 2) return;
        if (s == 0) {
            if (iface_member_init.cmpxchgStrong(0, 1, .acquire, .acquire)) |_| continue;
            iface_member_arena = std.heap.ArenaAllocator.init(std.heap.page_allocator);
            iface_member_init.store(2, .release);
            return;
        }
...
fn ifaceMemberLock() void {
    while (iface_member_mu.cmpxchgWeak(0, 1, .acquire, .monotonic)) |_| {
        std.atomic.spinLoopHint();
    }
}

fn ifaceMemberUnlock() void {
    iface_member_mu.store(0, .release);
}

/// Same contract as `collectInterfaceMembers`, but the result is allocated in
/// a process-lifetime arena and reused across calls. Callers must NOT free
/// the returned slice. Builtin interface names are static `@embedFile`-derived
/// strings, so using them as hashmap keys without duping is safe.
fn collectInterfaceMembersCached(iface: BuiltinInterface) !?[]const InterfaceMember {
    ifaceMemberInit();

    ifaceMemberLock();
    if (iface_member_cache.get(iface.name)) |hit| {
        ifaceMemberUnlock();
        return hit;
    }
    ifaceMemberUnlock();

    const arena = iface_member_arena.allocator();
    const members = (try collectInterfaceMembers(arena, iface)) orelse return null;

    ifaceMemberLock();
    defer ifaceMemberUnlock();
    // Double-check after re-lock — another thread may have populated this
    // entry while we were computing. Drop our copy on race (arena absorbs it).
    if (iface_member_cache.get(iface.name)) |hit| return hit;
    iface_member_cache.put(arena, iface.name, members) catch return members;
    return members;
}
...
/// `_N` → N for tuple element access (mirrors `comptime/infer.zig`'s
/// `tupleMemberIndex` — kept duplicated to avoid widening the compiler-core
/// public surface for a single helper). Returns null when `member` is not
/// `_<digits>`.
