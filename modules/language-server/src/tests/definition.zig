...
// ── DG7 — imported symbol jumps to its defining module ──────────────────────
...
/// §E E1 — `recv._N` (tuple element access) resolves to the Nth element's
...
test "definition: interface assoc-fn cross-module — Iface.method jumps to default fn (E2)" {
    const gpa = std.testing.allocator;
    ...
    // Cursor on `show` in `Show.show(42)` → `default fn show` in dep.
    const cursor = posOf(main_src, "Show.show", 1, "Show.".len);
    const empty: []const h.comptime_pipeline.TypedBinding = &.{};
    const td = try engine.definitionMember(gpa, h.TEST_URI, main_src, cursor, tokens, empty, &others);
    defer if (td) |t| switch (t) {
        .location => |loc| gpa.free(loc.uri),
        .builtin => {},
    };
    ...
    try std.testing.expect(td.? == .location);
    try std.testing.expectEqualStrings(dep_uri, td.?.location.uri);
    try std.testing.expectEqualStrings("show", sliceAt(dep_src, td.?.location.range));
}

test "definition: tuple element chain — t._0.field jumps to field decl on element type (E1)" {
    const gpa = std.testing.allocator;
    var c = try h.compile(gpa, tuple_source);
    defer c.deinit(gpa);
    const bindings = c.bindings() orelse return error.CompileFailed;

    var arena = std.heap.ArenaAllocator.init(gpa);
    defer arena.deinit();
    const tokens = try h.tokenize(arena.allocator(), tuple_source);

    // Cursor on `value` in `pair._0.value` → `value:` field decl on `Box`.
    const cursor = posOf(tuple_source, "pair._0.value", 1, "pair._0.".len);
    const td = try engine.definitionMember(gpa, h.TEST_URI, tuple_source, cursor, tokens, bindings, &.{});
    defer if (td) |t| switch (t) {
        .location => |loc| gpa.free(loc.uri),
        .builtin => {},
    };
