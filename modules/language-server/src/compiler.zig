/// Incremental compilation wrapper for the LSP.
///
/// Drives `comptime_pipeline.compile()` with runtime `.none` so type
/// diagnostics surface without executing external code.
...
pub const LspCompiler = struct {
    ...
    /// Compile a list of in-memory modules using type inference only. When
    /// `eval_root` is set, template bodies (incl. `@ExprCustom`) are
    /// expanded via `node` so their `CustomNode` trees reach `custom_ast`.
    /// The caller owns the result — call `result.deinit(gpa)` when done.
    pub fn compile(
        self: *LspCompiler,
...
/// (uri, source) pair passed to compilation.
...
/// Result of a compilation session.
pub const CompileResult = struct {
    ...
    /// Returns LSP diagnostics for the given URI.
    ...
    pub fn diagnosticsFor(
        ...
        for (self.session.outputs.items) |output| {
            ...
            switch (output.outcome) {
                ...
                .validationError => |err| {
                    // ComptimeError carries .loc: ast.Loc (1-based line/col)
