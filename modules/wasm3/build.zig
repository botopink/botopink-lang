...
pub const wasm3_cflags = [_][]const u8{
    "-std=c11",
    "-Os",
    // wasm's integer arithmetic wraps, and wasm3 implements it with plain C
    // signed arithmetic: `-fwrapv` makes that defined, and Zig's Debug-mode
    // UBSan (on by default for C) would otherwise trap on `i32.mul` overflow.
    "-fwrapv",
...
/// `persistent_wat.zig` calls into the C API directly. Brings in the C sources,
