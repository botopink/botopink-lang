...
Compile flags: `-std=c11 -Os -fwrapv -fno-sanitize=undefined` (the last two: wasm integer arithmetic wraps, and Zig's Debug UBSan would trap on it). We do **not** set any of `d_m3HasWASI`/`d_m3HasMetaWASI`/`d_m3HasUVWASI` — `m3_api_wasi.c`, `m3_api_meta_wasi.c` and `m3_api_uvwasi.c` compile to nothing under those guards. The modules botopink runs import nothing: see `modules/compiler-core/src/comptime/runtime/persistent_wat.zig`.
