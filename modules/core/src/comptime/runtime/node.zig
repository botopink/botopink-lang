...
fn buildScript(allocator: std.mem.Allocator, entries: []const eval.ComptimeEntry) ![]u8 {
    ...
    for (entries, 0..) |e, i| {
        ...
        switch (e.expr.kind) {
            .@"comptime" => |inner| {
                try writeExprJs(bw, inner.*);
            },
            .comptimeBlock => |cb| {
                for (cb.body) |stmt| {
                    switch (stmt.expr.kind) {
                        .@"break" => |y| if (y) |yp| {
                            try writeExprJs(bw, yp.*);
                            break;
                        },
                        else => {},
                    }
...
fn writeExprJs(bw: anytype, te: ast.TypedExpr) !void {
    switch (te.kind) {
        .numberLit => |n| try bw.writeAll(n),
        .stringLit => |s| try writeJsString(bw, s),
        .add => |b| {
            try bw.writeByte('(');
            try writeExprJs(bw, b.lhs.*);
            try bw.writeAll(" + ");
            try writeExprJs(bw, b.rhs.*);
            try bw.writeByte(')');
        },
        .sub => |b| {
            try bw.writeByte('(');
        ...
        .arrayLit => |arr| {
            ...
            for (arr, 0..) |item, i| {
                ...
                try writeExprJs(bw, item);
            }
            try bw.writeByte(']');
        },
        .@"comptime" => |e| try writeExprJs(bw, e.*),
        .@"break" => |y| if (y) |yp| try writeExprJs(bw, yp.*),
        else => try bw.writeAll("undefined"),
    }
}
