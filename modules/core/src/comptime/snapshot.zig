...
fn extractFnBody(
    ...
    for (stmts) |stmt| {
        try body.append(allocator, .{ .source = extractLine(src, stmt.expr.loc.line) });
    }
    return body.toOwnedSlice(allocator);
}

fn buildCaseExpr(allocator: std.mem.Allocator, expr: ast.TypedExpr) !CaseExpr {
    const case_ = expr.kind.case;
    ...
    return CaseExpr{
        .ast = "case",
        .param = try typeNameOf(allocator, case_.subject.type_),
        .match = try allocator.dupe(CaseArm, arms.items),
        .return_type = try typeNameOf(allocator, expr.type_),
    };
}

fn buildCallExpr(allocator: std.mem.Allocator, expr: ast.TypedExpr) !CallExpr {
    const call_ = expr.kind.call;
    var params: std.ArrayList(CallParam) = .empty;
    defer params.deinit(allocator);
    for (call_.args) |arg| {
        try params.append(allocator, .{
            .name = arg.label,
            .value = try typeNameOf(allocator, arg.value.*.type_),
        });
    }
    return CallExpr{
        .ast = "call",
...
fn buildCaseArm(allocator: std.mem.Allocator, body: ast.TypedExpr) !CaseArm {
    const retType = try typeNameOf(allocator, body.type_);
    switch (body.kind) {
        .lambda => |lam| if (lam.params.len == 0) {
            var items: std.ArrayList(StmtType) = .empty;
            defer items.deinit(allocator);
            for (lam.body) |stmt| {
                try items.append(allocator, .{ .return_type = try typeNameOf(allocator, stmt.expr.type_) });
            }
            return CaseArm{
                .ast = "block",
