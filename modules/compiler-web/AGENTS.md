...
| `index.html` | Two samples (a plain program; a decorator + a template), the four targets, a Worker running `glue.js`. On the `wasm` target the page runs what it compiled (the `run` op) and shows its output in the run pane. After `botopink.wasm` and `glue.js` load, no request leaves the page. The comptime sample shows today's answer on a wasm host: the located refusal of `comptime/runtime/runtime.zig`, until step 2 lands the wat runtime. |
