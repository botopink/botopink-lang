...
- Comptime evaluation: a decorator or template is refused on this host with
  the runtime named. commonJS/wasm evaluate on the wat runtime (decision 84) —
  its lowering, linker and term library are compiled in, but the engine (the
  page's `WebAssembly.instantiate`, behind a host import) is front 18 step 5's
  comptime half; erlang/beam evaluate on the BEAM, which a page never has.
