...
fn dispatch(init: std.process.Init) !u8 {
    ...
    if (std.mem.eql(u8, cmd, "build")) {
        return build_cmd.run(gpa, io, try parseBuildOpts(args[2..]), env_map);
    }

    if (std.mem.eql(u8, cmd, "check")) {
        return check_cmd.run(gpa, io, env_map);
    }

    if (std.mem.eql(u8, cmd, "run")) {
        return run_cmd.run(gpa, io, try parseRunOpts(gpa, args[2..]), env_map);
    }

    if (std.mem.eql(u8, cmd, "test")) {
        return test_cmd.run(gpa, io, try parseTestOpts(args[2..]), env_map);
    }
