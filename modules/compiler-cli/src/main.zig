...
test "migrate takes no subcommand: `effects` is a positional, refused" {
    var d: ArgDiag = .{};
    try std.testing.expectError(error.UnexpectedArgument, parseMigrateOpts(argv(&.{"effects"}), &d));
    try std.testing.expectEqualStrings("effects", d.token);
}
