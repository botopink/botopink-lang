...
| `run.zig` | `botopink run [--target <t>] [--module <name>] [--out <dir>] [-- <args>]` | Builds into `--out`, then spawns the runner for the target on `<out>/<module>.<ext>` (`node` / `escript` / `wasmtime`). `beam` only writes the `.S` artifact and prints the `erlc +from_asm` hint. **`escript` reaches one module only** — a multi-module erlang program is `undef` at run time with correct emitted code; see "the erlang runner reaches one module" in the parent `AGENTS.md`. The file is front `13-module-identity`'s. |
