...
├── diagnostics.zig    ← shared compile diagnostics: lex/parse preflight, named-set guard, located rendering
...
| `diagnostics.zig` | Shared by `build`/`check`/`test`: `preflight` lexes and parses every module and renders a located lex/parse error (the module is left out of compilation); `missingOutputs` compares the named module set given to `codegen.generateWith` with the named set it returned; `explainFailures` re-runs comptime on the failure path to render each type/validation error with file, line and excerpt; `reportFailedModules` / `reportOrphans` print the summary lines. `printLexError` / `printParseError` are reused by `format`. |
