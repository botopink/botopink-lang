...
/// The error type of a fallible wrapper value: `name<T, E>` with `E` not `any`.
fn fallibleError(t: ?*Type, names: []const []const u8) ?*Type {
    ...
    switch (ty.*) {
        .named => |n| {
            ...
            if (!hit or n.args.len < 2) return null;
            const e = n.args[1].deref();
    ...
    for (fa.sites) |site| {
        ...
        switch (site.kind) {
            .await_ => {
                ...
                const err = fallibleError(fa.typeAt(site.operand), &.{"Future"}) orelse continue;
                _ = err;
            ...
            .for_ => {
                ...
                _ = fallibleError(t, &.{ "ResultGenerator", "FutureGenerator" }) orelse continue;
            ...
            .next_call => {
                ...
                _ = fallibleError(fa.typeAt(site.operand), &.{ "ResultGenerator", "FutureGenerator" }) orelse continue;
