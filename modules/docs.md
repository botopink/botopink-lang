...
├── lib-test-runner/     → `botopink-lib-test`       (shells out to `botopink`)
└── bpmp/                → `bpmp` package manager    (spawns `botopink`)
```
...
- **`lib-test-runner`** ships `botopink-lib-test`, the per-lib / per-backend
  test matrix gate. It shells out to the `botopink` CLI rather than linking
  `compiler-core`, so the runner stays self-contained.
- **`bpmp`** ships the `bpmp` package manager / toolchain manager. It spawns
  `botopink` rather than linking it; its source of truth for the on-disk
  layout is its own `AGENTS.md`.
...
cd modules/<package> && zig build run       # run (cli + lsp + bpmp)
cd modules/<package> && zig build test      # tests (core + lsp + lib-test-runner + bpmp units)
```

The workspace root `build.zig` wires the executables (CLI, LSP, lib-test-runner,
bpmp) together; per-package builds remain independent so individual modules can
...
| No standalone JS/Erlang/BEAM/WASM compiler | All four backends are produced natively in Zig under `compiler-core/src/codegen/` |
