...
| Record (`type`) | Field list in parentheses, no `val` → `type Point(x: i32, y: i32)`, always written — `type Empty()` for a record with no fields (decision 137); compact without a trailing comma (even past the width), open one field per line with the trailing comma when the source had one or a field carries a `//` comment; field annotations and defaults inline; no body when there are no methods; ` implement B` after the field list |
