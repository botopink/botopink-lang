...
| `comptime.zig` | Comptime façade — `ComptimeSession`, `compile` (which selects the comptime runtime for its pass: `comptime/runtime/runtime.zig` `forTarget(target_name)`, decision 84), `compileTypesOnly`, `evaluateComptime`, `ExprTypeLog` / `setExprTypeLog` (tooling: install `infer.ExprTypeLog` for one inference and read the type of every expression by file and location — `botopink migrate effects`), `setEffectMigration` (the migration-only mode of `botopink migrate effects`, decisions-pending 24-d: the parser reads the removed effect surface as its new spelling and the checker types the listed files with the old meaning; thread-local, set only by the codemod), `bundled_packages` / `bundledPackage(name)` / `importsPackage(source, name)` (the bundled-package table `build.zig` generates — `std` plus the libraries it ships, decisions 115–117; the core names none of them, and a non-std one is ordinary code here: the CLI and the LSP load its embedded modules as `<pkg>/<stem>`), `expandStdImports` (the embedded std modules a build needs: each `from "std"` import, and `std/beam` for an erlang/beam build of a module that declares a module `var` — front 17, the lowering calls into it), `registerStdlib` (`registerReflectionPrelude` registers the `@Decl` cluster + `SourceLocation` into the global env AND into the scratch env each std module is inferred in, so a std signature may name a prelude record — `snapshots.path(loc: SourceLocation)`; the prelude enum `YieldStep<T>` (decision 122) is registered beside them, and `withYieldStepDecl` splices its private declaration into a module that names it or calls `.next()` on a sequence, as `withSourceLocationDecl` does for the record — `comptime/AGENTS.md`). A module that does not lex or parse is an `Outcome.parseError` carrying a located `SyntaxError` (`.lex` / `.parse`), never a session-wide error |
