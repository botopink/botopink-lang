...
| `call $undefined` | `validateModule` walks every `call` against the module's own functions, its imports and its declared `externs`, before a byte is written. A **runtime helper** is stronger still: `Builder.helper` is the only way to obtain its symbol, and it marks the helper's group for emission in the same act — "called" and "defined" are one operation. |
...
- `../wat.zig` — builds every instruction, function, global and data segment as
  ...
  `renderModule`. `emitFnWat` renders the accumulated items as bare forms.
