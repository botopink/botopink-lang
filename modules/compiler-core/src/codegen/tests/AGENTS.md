...
decision 140's audit — every `#[@External.Erlang(…)]` template std, its
...
what holds it.
The four `codegen ---- use … is a plain call` cells of `features.zig` record decisions 88 and 104 (1.0.10-beta, fronts 19 and 21): `val c = use state(0)` is `const c = await state(0)` on commonJS, where every `-> @Component` body (hook or component) is an `async function`, and the bare call on erlang, beam and wasm — their components return `@Component<C, T>`, the effect that lets a body activate a hook (decisions 118, 128). They replaced the `… to useState` / `… infers dependency array` / `… empty deps` cells, whose snapshots recorded the React rename.
