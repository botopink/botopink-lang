...
test "js: loop ---- two-parameter loop threads reassigned vars out" {
    ...
    // KNOWN: `weigh` is 140; commonJS prints 80 (the `1..` start is ignored —
    // 04-js-bridges) and beam prints nothing (01-beam).
    try h.assertJsSingle(std.testing.allocator, @src(),
...
test "js: lambda ---- a local closure reassigning outer vars threads them out" {
    ...
    // its closures do not thread reassigned outer vars out yet (1.0.4-beta
    // 01-beam).
    try h.assertJsSingle(std.testing.allocator, @src(),
