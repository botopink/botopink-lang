...
const Emitter = struct {
    ...
    fn emitFn(self: *Emitter, f: ast.FnDecl) !void {
        ...
        for (f.params) |p| {
            ...
            if (self.resolveRecordName(tn)) |rty|
                try self.local_types.put(p.name, rty);
    ...
    fn emitMemberFn(self: *Emitter, owner: []const u8, m: ast.InterfaceMethod) !void {
        ...
        const has_result = bodyYieldsValue(body);
        self.resetFnState("i32");
        ...
        if (has_result) try self.w(" (result i32)");
        try self.w("\n");
        const try_count = countTrys(body);
        for (0..try_count) |i| {
            try self.fmt("    (local $_try{d} i32)\n", .{i});
        }
        const mem_count = self.countMems(body);
        for (0..mem_count) |i| {
            try self.fmt("    (local $__mem{d} i32)\n", .{i});
        }
        try self.emitLocalDecls(body);
        try self.emitBody(body, "i32");
        try self.w("  )\n");
    }
        ...
        switch (stmt.expr) {
            ...
            .binding => |b| switch (b.kind) {
                ...
                .assign => |a| switch (a.target) {
                    ...
                    .fieldAccess => |fa| {
                        ...
                        if (off_opt) |off| switch (a.op) {
                            .assign => {
                                try self.lowerExpr(fa.receiver.*);
                                try self.lowerExpr(a.value.*);
                                if (off == 0)
                ...
                if (is_last) {
                    ...
                    try self.lowerExpr(stmt.expr);
                    // §wat-refactor F1 — void-call / void-builtin stmts leave
                    // nothing on the stack, so the conservative `drop` would
                    // underflow at runtime. The classifier `producesValue`
                    // detects them syntactically (call to `@print`/`@panic`/
                    // `@todo`/builtin stub) so the drop only fires when there
                    // is something to discard.
                    if (producesValue(stmt.expr)) try self.w("    drop\n");
                }
            },
        }
    }

    /// True when `e` is known to push exactly one i32 onto the wasm operand
    /// stack at the end of `lowerExpr`. Conservative: defaults to true unless
    /// `e` is a builtin call recognised as void (`@print`, `@panic`, `@todo`,
    /// the unhandled `;; builtin stub` arm of `lowerBuiltin`). A non-builtin
    /// `.call` is assumed to produce one value — most user fns do, and the
    /// statement loop falls back to dropping an extra value rather than
    /// risking an underflow on a stub fn (the cost of an unused drop is a
    /// validator warning at worst). Pure expression shapes (`literal`,
    /// `identifier`, `binaryOp`, …) always produce one value.
    fn producesValue(e: ast.Expr) bool {
        return switch (e) {
            .call => |c| switch (c.kind) {
    ...
    fn emitBranchBody(self: *Emitter, body: []const ast.Stmt, as_stmt: bool) anyerror!void {
        if (body.len == 0) {
            if (!as_stmt) try self.fmt("        {s}.const 0\n", .{self.cur_result});
            return;
        }
        for (body, 0..) |stmt, idx| {
            const is_last = idx == body.len - 1;
