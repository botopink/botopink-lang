...
**What it refuses, by name — each is the whole declaration running from
Erlang source instead (the `COMPTIME ERLANG` fallback, first line naming the
construct):** from the reader, `receive`, `maybe`, the old `catch Expr`,
...
auto-imports (`erlc` refuses it too — the fallback reproduces its message).
**Measured: none of these occurs in the suite's comptime modules** (37
...
`tests/language/run.sh --target all`), nor in 377 of 389 modules on disk from
