...
├── beam/             ← BEAM term model + shared emitters (`term.zig`, `erl_emitter.zig`, `beam_emitter.zig`) — see [`beam/AGENTS.md`](beam/AGENTS.md)
...
| `beam/` | **BEAM term model + emitters** shared by `erlang.zig`, `beam_asm.zig` and the comptime evaluators: `term.zig` (`Term` value model), `erl_emitter.zig` (Erlang source: atom quoting incl. reserved words, variables, binaries from raw bytes or from lexemes, whole terms), `beam_emitter.zig` (`.S` operands: `{atom,…}`/`{integer,…}`/`{float,…}`/`nil`/`{literal, <term>}`, `{move,…}`). Both backends take `atomName`/`fnAtom`/`erlangVar`/string-literal escaping from here — one quoting rule for `.erl` and `.S`. See [`beam/AGENTS.md`](beam/AGENTS.md) |
