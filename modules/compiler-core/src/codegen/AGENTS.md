...
- **Comptime modules:** `emitComptimeModule(alloc, name, program, .{ host_enums,
  ...
  exports, `-import` or helpers — the `COMPTIME ERLANG` section of a body the BEAM
  lowering refused, not a compilable module), `forms` (`[]erl_ast.Form`) are rendered after the
