...
├── hostMethods.zig   ← host-backed methods: a `declare fn` + `#[@External.*]` inside a `type` body — which backends bind it, and the call-site refusal
...
| `commonJS.zig` | CommonJS backend — builds `js/js_ast.zig` nodes, rendered by `js/js_emitter.zig`. See [commonJS](#commonjs) below A `case` used as a statement whose arms `return` runs its arrow with `case_return_box`: an arm's `return` answers `{ __bp_return, value }` and the statement after the call returns it (`caseStmtWithReturns`). A pattern written `Result.Ok`/`Result.Error` (the transform's qualification) is the `@Result` test even beside a user enum's `Error`. |
| `erlang.zig` | Erlang source emitter. See [erlang](#erlang) below A `.by_value` method call — and an `.unplaced_type` one on a record or enum of the program, unless it names a function-typed field — goes through `'__bp_method'/3` (the value's tag, or a host-built map's function). A `return` nested in a statement whose value is discarded (an `if` inside an `if` block, a `case` arm) throws to the function's guard (`in_nested_return`, `ast.exprReturns`). The array `length` is `erlang:length/1`, so a type's own `length/1` method never answers it. The test runner's loader starts at the run's output root, so a test file in a folder loads the whole tree. |
| `beam_asm.zig` | BEAM Assembly `.S` emitter, assembled with `erlc +from_asm`. See [beam_asm](#beam_asm) below A `.by_value` method call (and an `.unplaced_type` one on a program type) goes through the per-module `'-bp_method-'/3` helper (`ensureMethodHelper`: a map applies its own function, a record its tag's module). |
...
- **A host answer is adopted into the record its declaration names**
  (`adoptHost`, the `__bp_adopt(v, C, path)` prelude helper): a host function
  or host-backed method whose return type names a record class the module can
  reach — directly, through `?T`, element by element through `T[]` /
  `Array<T>` (`a`), or inside an `@Result`'s ok side (`r`) — has its answer
  given the class's prototype, fields kept, at the call site in the owning
  module, in the exported template wrapper and in the host method's body. A
  plain object used to stay one: its fields read, but it had none of the
  record's methods (a host-backed `Regex.matches` on the object `compile`
  built was `… is not a function`) and printed as `%O`. An instance of the
  class and `null` pass through. `@Task` is not looked through, and a
  `(module, symbol)` alias another module imports bypasses it (the consumer
  calls the host symbol directly). The erlang twin is `adoptHostResult`.
  Pinned by `tests/language/run/external_method_on_host_record`.
...
- **Comptime modules:** `emitComptimeModule(alloc, name, program, .{ host_enums,
  ...
  exports, `-import` or helpers; the evaluators keep its `main/1` argument
  comments for the listing, not a compilable module), `forms` (`[]erl_ast.Form`) are rendered after the
