...
## Async, generators & iterators (`#[@<effect>]`, `await`, `yield`, `loop await`)

A `#[@<effect>]` annotation marks a function whose return implements
`@Future<_>` (`#[@future]`), `@Iterator<_>` / `@AsyncIterator<_, _>`
(`#[@iterator]` / `#[@asyncGenerator]`), `@Generator<_>` (`#[@generator]`),
`@Result<_, _>` (`#[@result]`), or `@Context<_, _>` (`#[@context]`). It sets
`FnDecl.effect`; `await`/`yield` then follow from the return type. (The
`*fn` prefix that previously carried the same intent was removed in
v0.beta.19; see the `deprecated-star-fn` parser diagnostic.)
...
A `#[@<effect>]` fn must have a body (the annotation is meaningless without
one). Anonymous function expressions never carry an effect annotation —
`fn(...) { ... }` and `{ ... -> ... }` are the only anon-fn forms.
