...
│                     `effect-annotation-removed` with the `iter` / `stream` fix-it; a removed wrapper
│                     name in a type is `effect-type-removed` (`types.zig`) — always, there is no mode
│                     that reads the old surface (decision 131). `throw new X(…)` is
