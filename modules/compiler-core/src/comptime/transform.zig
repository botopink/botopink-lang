...
fn scanExpr(agg: *Aggregator, fn_decls: std.StringHashMap(ast.FnDecl), comptime_arrays: std.StringHashMap([]const ast.TypedExpr), expr: anytype) ScanError!void {
    switch (expr) {
        ...
        .binaryOp => |b| switch (b.kind) {
            .add, .sub, .mul, .div, .mod, .eq, .ne, .lt, .gt, .lte, .gte, .@"and", .@"or" => |op| {
                scanExpr(agg, fn_decls, comptime_arrays, op.lhs.*) catch return ScanError.OutOfMemory;
                scanExpr(agg, fn_decls, comptime_arrays, op.rhs.*) catch return ScanError.OutOfMemory;
            },
        },
        ...
        .function => |func| switch (func.kind) {
            ...
            .fnExpr => |f| {
                for (f.body) |s| scanExpr(agg, fn_decls, comptime_arrays, s.expr) catch return ScanError.OutOfMemory;
            },
        },
...
fn rewriteExpr(agg: *Aggregator, fn_decls: std.StringHashMap(ast.FnDecl), comptime_arrays: std.StringHashMap([]const ast.TypedExpr), expr_ptr: *ast.Expr) ScanError!void {
    switch (expr_ptr.*) {
        ...
        .binaryOp => |*b| switch (b.kind) {
            .add, .sub, .mul, .div, .mod, .eq, .ne, .lt, .gt, .lte, .gte, .@"and", .@"or" => |op| {
                rewriteExpr(agg, fn_decls, comptime_arrays, op.lhs) catch return ScanError.OutOfMemory;
                rewriteExpr(agg, fn_decls, comptime_arrays, op.rhs) catch return ScanError.OutOfMemory;
            },
        },
        .collection => |*col| switch (col.kind) {
            .arrayLit => |al| {
        ...
        .function => |*func| switch (func.kind) {
            ...
            .fnExpr => |f| {
                for (f.body) |*s| rewriteStmt(agg, fn_decls, comptime_arrays, s) catch return ScanError.OutOfMemory;
            },
        },
        .identifier => |*id| switch (id.kind) {
