...
pub const Entry = struct {
    ...
    name: []const u8,
    /// Generated Erlang: the lowered function and `main/0`.
    erl: []const u8,
...
pub fn render(allocator: std.mem.Allocator, buf: *std.ArrayListUnmanaged(u8), entries: []const Entry) !void {
    for (entries) |e| {
        ...
        try buf.print(allocator, "----- COMPTIME ERLANG -- {s} {s}\n```erlang\n", .{ kind, e.name });
        try appendBlock(allocator, buf, e.erl);
...
test "trace: json replies are re-indented, error texts kept" {
    ...
    const out = (try renderAlloc(alloc, &.{
        .{ .kind = .template, .name = "shout", .erl = "shout(Q) ->\n    ok.\n", .reply = "{\"kind\":\"code\",\"source\":\"1\"}" },
        .{ .kind = .decorator, .name = "route", .erl = "route(D) -> ok.", .reply = "the decorator body raised: badarg" },
    })).?;
