...
| `trace.zig` | `Entry{kind: template/decorator, name, erl, reply}` — one per `template_eval`/`decorator_eval` run, appended to `Env.comptimeTraces` (surfaced as `OkData.comptime_traces`; `analyzeModule` keeps pass-1 decorator traces across the `@emit` re-analysis). `erl` is the `listing` build of the module (lowered body + `main/1`) followed by `main/1`'s argument as comment lines — the input half of the evaluation, which is no longer inside the module; `reply` is the JSON `main/0` printed, or `compile error: …` / `runtime error: …`. `render`/`renderAlloc` write the `COMPTIME ERLANG` and `COMPTIME REPLY` sections (JSON re-indented, **objects' keys sorted** — `runtime/reply_order.zig`: the order a runtime's map iterated in is not part of the reply, and the BEAM's is its atom table's). |
