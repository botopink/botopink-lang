...
| `snapshot.zig` | Snapshot helpers. Section order: `SOURCE CODE`, `COMPTIME ERLANG` / `COMPTIME REPLY` per runtime evaluation, `COMPTIME VALUES` + `BOTOPINK TRANSFORM CODE` (when comptime vals exist), `TYPED AST JSON`. |
