...
| `template.zig` | `@Expr` template infrastructure: `CapturedExpr` (argument bound to a `comptime p: @Expr<T>` param, captured unevaluated with provenance), `PlainArg` (`{ paramName, jsValue }` — a non-`@Expr` param that received a literal), `ScopeSnapshot` (origin scope: caller's top-level decls + imports, `toJsonAlloc`), `contextJsonAlloc`, `CustomNode` + `parseCustomNode` (JSON) / `parseCustomNodeFromTree` (native `CustomNodeTree`), and `mapSpanToLoc`/`failDiagnostic` (rustc-style `fail`/`failAt` diagnostics inside the caller's `"""…"""`). With contiguous text `mapSpanToLoc` counts newlines up to `span.start`; for holed multiline templates it falls back to `capture.loc.line + span.line - 1` (`span.line` is 1-based, line 1 = opening `"""` line). |
| `template_eval.zig` | Runs a template body that the V1 classifier cannot reduce. Current implementation builds synthetic botopink source from the body (`emitBpBody`/`emitBpStmt`, plain args as `val` bindings), runs it through `comptime.compile` + `codegen/erlang.zig`, writes the `.erl` under `.botopinkbuild/tmp/template_<hash>/`, and executes it with `runtime/persistent_erl.zig`. The JSON result is parsed into `ErlTemplateResult` and returned as `Outcome` — `code`, `value` (`TypedValue`), `capture`, `custom` (`CustomNodeTree`), `fail`, `err`. **Known broken:** the synthetic module fails to parse (`parse failed for module 'template_body'`), so runtime-backed expansion currently returns `EvalFailed`. |
| `decorator_eval.zig` | Runs a decorator body over the declaration it annotates. Input is a native `DeclHandle` (`kind`/`name`/`fields: []FieldHandle`/`methods`/`returnType`/`annotations`) built by `infer.zig`; `handleToTerm` converts it to a `codegen/beam/term.zig` `Term` map (`kind` as atom, names as binaries, annotation args as raw lexemes), written via `codegen/beam/erl_emitter.zig`. The body is currently lowered by a hand-written Erlang emitter in this file (`emitExpr`/`emitStmt`/`buildErlModule`; `fail`/`@emit`/`@compilerError` become module-local host fns) and run through `persistent_erl.eval`; the JSON result becomes `Outcome` — `ok` (`@emit` contributions), `fail{message, span}`, `err`. Replacing the hand-written emitter with `codegen/erlang.zig` `emitComptimeModule` is in progress. |
...
`captures.len + plainArgs.len == tfn.params.len` (plain args serialized by
`literalToJsAlloc`), memoizes by callee + capture texts + scope JSON + plain arg
...
(`TypedValue` → literal / array / anonymous record; `literalFromJson` is the
unused `std.json.Value` variant); `fail` → `failDiagnostic`.
...
- `@compilerError(message)` — generic compile-time rejection usable from a
  ...
  `__compilerError`; the decorator Erlang module defines `compilerError/1`
  (throws `comptime_fail`, reported like `decl.fail`); `template_runtime.bp`
  defines it via `failRaw`.
...
- **Invocation:** `invokeDecorators` (skipped when `env.skipDecoratorInvoke` or
  ...
  become `PlainArg`s). `fail`/`err` become a `TypeError` (coarse loc; the
  message carries the detail); `ok` appends `@emit` sources to
  `env.contributions`.
