...
| `tests.zig` | Barrel aggregating `tests/<feature>.zig` (plus the inline tests of `eval.zig`, `primOpTemplate.zig`, `diagnostics.zig`, `template_eval.zig`, `decorator_eval.zig`); harness in `tests/helpers.zig`. |
