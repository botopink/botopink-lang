...
├── dsl_hygiene.zig    ← decision 112: a template's built code — the library's names resolve in the library
...
| `diagnostics.zig` | Stable diagnostic-code constants (R1–R21, RF1–RF5, RI1–RI6, RC1–RC6, RG1–RG4, D1–D6, `std-unsupported-on-target`, `src-takes-no-arguments`, `unknown-builtin`, `unknown-primitive-method`, `unknown-annotation`, `effect-try-without-fallible-channel`, `option-expect-removed`) plus the `all_codes` table. Messages live at the firing site. |
...
`refuseFallingOffTheEnd` (after `inferFnDecl`'s body) refuses a plain `fn` whose written return type
...
name the module does not declare is found among the types it IMPORTS: `registerExports`
...
`5e-324`, `1e3`; `0x1E` stays an integer) — it was typed `i32`. Cells:
`tests/language/run/integer_division_truncates.bp`, `tests/language/run/float_literal_spellings.bp`.
