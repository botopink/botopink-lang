...
| `infer_errors.zig` | Inference type errors (`infer error: …`), the `val destructure: …` cells of 01 R5 (a one-variant / record pattern binds typed, a refutable one is `refutable-val-pattern`), including `src-takes-no-arguments` and `unknown-builtin` (with and without a near name). The decision 38 / front 17 rows (`val` assignment, `@BeamMemory` validation) assert the message by content through `typeErrorMessage` — no cell under `snapshots/comptime/errors/`. |
