...
| `effects.zig` | throw / context / `@Result` effect checking. The `context:` cells carry `#[@use]` on every body that writes `use` and on every hook (decisions 88, 102, 104 of 1.0.10-beta); `use without #[@use] on a -> Element body` pins `use-without-context-effect`, `#[@use] hook and component compose` the two wrappers, `#[@future] fn -> @Future<Element> does not activate` the revocation of decisions 89/90 and `#[@use] fn -> @Component<Element> uses and awaits` the chain that replaces them; the wrapper refusals are `… (bare owner) is effect-missing-wrapper`, `… -> string is effect-wrapper-mismatch`, `#[@use] on a @Component whose type owns no context`, `use of a component is refused` and `a fn returning @Use without #[@use]`. Front 19 step 3: `use tuple destructure binds element types` pins that `val #(shown, push) = use optimistic(…)` binds `R`'s elements (its `error:` sibling reds on `push("x")`), and `use tuple destructure arity mismatch` / `… whose R is not a tuple` pin `use-tuple-arity`. |
