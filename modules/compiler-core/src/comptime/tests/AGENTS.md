...
| `infer_errors.zig` | Inference type errors (`infer error: …`), the `val destructure: …` cells of 01 R5 (a one-variant / record pattern binds typed, a refutable one is `refutable-val-pattern`), the `chained call: …` and `leading-dot call: …` cells (a callee carried in `calleeExpr`), the `behavior-typed field …` cells of 01 R4, the `type position: …` cells of 01 R8, the `associated fn: …` cell of 01 R6, including `src-takes-no-arguments` and `unknown-builtin` (with and without a near name). The decision 38 / front 17 rows (`val` assignment, `@BeamMemory` validation) assert the message by content through `typeErrorMessage` — no cell under `snapshots/comptime/errors/`. |
| `types.zig` | Types / type unification. |
| `variants.zig` | Variants, record update, patterns, `@print`, AST probes. The three `DOCUMENTED SKIP`s that pinned the parse error of `<Pattern> as <name>` (`Ok(v) as whole`) are gone with their `comptime/ast/` snapshots — decision 11 of 1.0.5-beta: the form is not part of the language, and a test kept alive only by its own skip is a promise the language is not making. |
| `narrowing.zig` | Null-check / case-variant / type-guard narrowing. |
| `exhaustiveness.zig` | `case` exhaustiveness + reachability errors. |
| `effects.zig` | throw / context / `@Result` effect checking — the return is the effect (decisions 118–128 of 1.0.10-beta). The `context:` cells return `@Component<C, T>` on every body that writes `use` and on every hook (decisions 88, 102, 104, 128); `use without @Component on a -> Element body` pins `use-without-context-effect`, `@Task fn -> @Task<Element> does not activate` the revocation of decisions 89/90; the wrapper refusals are `@Component on a @Component whose type owns another base` (`effect-wrapper-mismatch`), `@Component with one argument is refused` and `use of a component is refused`; a bare owner return (`-> Element`) activates nothing and is legal. The `chain:` cells pin `Component ⊃ Task`, `Stream ⊃ Task` and the fallible channel (`try` only where a layer of the return is a `@Result`, decision 121); `for over @Iterator<@Result<…>> hands over the @Result` pins decision 122 (no implicit `try`). Front 19 step 3: `use tuple destructure binds element types` pins that `val #(shown, push) = use optimistic(…)` binds `R`'s elements (its `error:` sibling reds on `push("x")`), and `use tuple destructure arity mismatch` / `… whose R is not a tuple` pin `use-tuple-arity`. |
