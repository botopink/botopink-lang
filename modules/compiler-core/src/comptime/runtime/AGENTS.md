...
- **The browser build carries neither runtime yet.** `modules/compiler-web/`
  builds compiler-core for `wasm32-wasi`; there `available(.beam)` and
  `available(.wat)` are both false — no spawn, and the page's
  `WebAssembly.instantiate` is not reached from the compiler yet — so both
  evaluators refuse with the missing runtime named, `persistent_erl.zig` is
  never analysed and wasm3 is never `@cImport`ed. The wat runtime's lowering and
  linker (`wat/`) and the embedded term library are in the browser build already.
