...
- **`erl.stderr.log` is a write-only debug log, by decision.** It holds erl's own
  ...
  truncated at every spawn, nothing in the compiler reads it back, and it is not
