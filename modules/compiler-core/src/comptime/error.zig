...
pub const TypeError = struct {
    ...
    pub fn message(this: TypeError, gpa: std.mem.Allocator) ![]u8 {
        return switch (this.kind) {
            ...
            .useNotAllowed => |r| std.fmt.allocPrint(gpa, "use-of-non-context-fn: `use` not allowed: function returns '{s}', which is neither `@Use<C, _>` nor `@Component<T>`", .{r}),
            .useNotContext => |e| std.fmt.allocPrint(gpa, "use-of-non-context-fn: `use` takes a hook: '{s}' is not a `@Use<C, _>`", .{e}),
            .contextMismatch => |m| std.fmt.allocPrint(gpa, "context-anchor-violation: function anchors at `{s}` but `use` returns @Use<{s}, _>", .{ m.fnBase, m.useBase }),
            .contextBaseMixed => |m| std.fmt.allocPrint(gpa, "context-anchor-violation: every `use` in one function resolves against the same ContextBase: this body's is `{s}`, fixed by the `use` on line {d}, and this one is @Use<{s}, _>", .{ m.anchorBase, m.anchorLine, m.useBase }),
