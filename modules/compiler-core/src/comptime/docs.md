...
`return`/`throw` inside an `#[@result] fn -> @Result<D, E>`. Inference records each such
...
|---|---|---|
| `#[@result] fn -> @Result<D, E>` (checked-Result effect) | `.result E` | thrown value must unify with `E`; `return`/`throw` construct `{ok}/{error}` values |
