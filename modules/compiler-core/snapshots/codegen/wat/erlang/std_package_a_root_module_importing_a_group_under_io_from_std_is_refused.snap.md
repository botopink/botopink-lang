...
%% external fn read -> erlang template
read(Name) ->
    (fun(__N) -> case os:getenv(binary_to_list(__N)) of false -> undefined; __V -> list_to_binary(__V) end end)(Name).

% Write `value` to env var `name`. The host APIs mutate the process

% env table in place.

%% external fn write -> erlang template
write(Name, Value) ->
    os:putenv(binary_to_list(Name), binary_to_list(Value)).

% Clear env var `name`. After this call, `read(name)` returns `null`.

%% external fn clear -> erlang template
clear(Name) ->
...
%% external fn args -> erlang template
args() ->
    [list_to_binary(__A) || __A <- init:get_plain_arguments()].
...
%% external fn vars -> erlang template
vars() ->
    (fun() -> [(fun(__E) -> [__K, __V] = string:split(__E, "=", leading), {list_to_binary(__K), list_to_binary(__V)} end)(__E) || __E <- os:getenv()] end)().
